...
#include "Vector - 1071710 - hw5.h" // include definition of class vector 
...
HugeInteger< T > HugeInteger< T >::operator-( HugeInteger< T > &op2 )
{
   ...
   for (int i = 0; i < subtrahendSize; i++) //減法 可能出現負數
   {
	   if (difference.integer.begin()[i] < op2.integer.begin()[i])
	   {
		   difference.integer.begin()[i] += 10;
		   for (int j = i; ; j++)
		   {

			   if (difference.integer.begin()[j + 1] != 0)
			   {
				   difference.integer.begin()[j + 1]--;
				   break;
			   }
			   else
			   {
				   difference.integer.begin()[j + 1] += 9;
				   continue;
			   }
		   }
	   }
