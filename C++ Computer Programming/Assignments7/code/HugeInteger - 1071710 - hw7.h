...
#include "Vector - 1071710 - hw7.h" // include definition of class template vector
#include "List - 1071710 - hw7.h"   // include definition of class template list
...
bool HugeInteger< T >::operator<( HugeInteger< T > &right )
{
	...
	int	size2 = right.integer.size();

	if (size1 > size2)
		return false;
	else if (size1 < size2)
		return true;
	for(;it2 != right.integer.rend(); it2++, it1++)
	{
...
HugeInteger< T > HugeInteger< T >::operator-( HugeInteger< T > &op2 )
{
   ...
   for (; it2 != op2.integer.end();++it2,++it3) 
   {
	   ...
	   {
		   (*it3) += 10;
		   for (it4=it3 ;it4!=difference.integer.end() ; it4++)
		   {
			   if (it4 == it3)
				   it4++;
			   if (*(it4) != 0)
   ...
   typename T::reverse_iterator it5 = difference.integer.rbegin();
   int count = 0;
   for (; it5 != difference.integer.rend();it5++)
   {
	   if ((*it5) == 0)
	   {
		   count++;
  ...
  HugeInteger zero;
   ...
   for ( ;it2!=op2.integer.end(); it2++)
   {
	   ...
	   for (int i = 0; i < record; i++)
		   it3++;
	   for (it1 = integer.begin(); it1!=integer.end(); it1++ , it3++)
		  (*it3) += (*it1) *(*it2);
   ...
   //®ø¹s
   typename T::reverse_iterator it5 = product.integer.rbegin();
   int count = 0;
   for (; it5 != product.integer.rend(); it5++)
   {
	   if ((*it5) == 0)
	   {
		   count++;
...
HugeInteger< T > HugeInteger< T >::operator/( HugeInteger< T > &op2 )
{
   ...
   typename T::iterator temp =remainder1.integer.begin();
   typename T::iterator divide = integer.begin();
   for ( ;temp!=remainder1.integer.end(); temp++,divide++)
   {
	   *temp = *divide;
   }
   ...
   typename T::iterator buffer1 = buffer.integer.begin();
   for (int i = 0; i < n; i++)
	   buffer1++;
   for (; divisor!=op2.integer.end(); divisor++,buffer1++)
	   (*buffer1) = (*divisor);
   ...
   typename T::reverse_iterator quotient1 = quotient.integer.rbegin();
   for (;quotient1!=quotient.integer.rend(); quotient1++)
   {
