...
{
	typename deque<Ty>::const_iterator leftNum = left.begin();
	typename deque<Ty>::const_iterator rightNum = right.begin();
	...
	{
		for (; leftNum != left.end();leftNum++,rightNum++ )
		{
			if (*leftNum!=*rightNum)
				return false;
		}
