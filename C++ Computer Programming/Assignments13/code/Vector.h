...
public:
   ...
   vector( const vector &right )
      : myData()
   {
	   int CapaLen = right.myData.myEnd - right.myData.myFirst;
	   int sizeLen = right.myData.myLast - right.myData.myFirst;
	   myData.myFirst = new value_type[sizeLen]();
	   for (int i = 0; i < sizeLen; i++)
	   {
		   myData.myFirst[i] = right.myData.myFirst[i];
	   }
	   myData.myLast = myData.myFirst + sizeLen;
   ...
   vector& operator=( const vector &right )
   {
	   if (this != &right) // avoid self-assignment
	   {
		   ...
		   vector::iterator it2 = right.begin();
		   resize(right.size());
   ...
   {
	   size_type orginalSize = size();
	   if (newSize > orginalSize)
	   {
		   if (newSize > capacity())
		   {
			   size_type capa = capacity() * 3 / 2;
			   if (capa < newSize)
				   capa = newSize;
			   pointer temp = new value_type[capa]();
			   for (int i = 0; i < orginalSize; i++)
				   temp[i] = myData.myFirst[i];
			   delete[] myData.myFirst;
			   myData.myFirst = &temp[0];
			   myData.myEnd = myData.myFirst + capa;
		   }

		   for (size_type i = orginalSize; i < newSize; i++)
			   myData.myFirst[i] = Ty();
   ...
   iterator erase( const_iterator where )
   {
	   ...
	   {
		   int sizeLen2 = where - myData.myFirst;
		   int sizeLen = myData.myLast - myData.myFirst;
		   for (int i = sizeLen2; i < sizeLen; i++)
		   {
